#!/bin/bash
# Compile, then run all ProgramBench test branches for paradigmxyz__solar.5190d0e.
#
...
INSTANCE=paradigmxyz__solar.5190d0e
...
"$PB/venv/bin/pip" install -q pytest pytest-timeout pytest-xdist pytest-rerunfailures pytest-dependency tomli libtmux pexpect
...
while read -r branch; do
    ...
    # Some test tarballs ship a broken `executable` symlink (-> target/release/...);
    # rm it first so cp writes a real binary instead of following the dead symlink.
    rm -f "$dir/executable"; cp executable "$dir/executable" && chmod +x "$dir/executable"
